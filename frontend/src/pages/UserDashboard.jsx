...
const PLAN_COLORS = { free: "#8a8496", pro: "#5a4bff", team: "#457b9d", enterprise: "#6a4c93" };
const PLAN_LABELS = { free: "Free", pro: "Pro", team: "Team", enterprise: "Enterprise" };

const UPGRADES = [
  { plan: "pro", label: "Pro", monthly: 249, yearly: 199, color: "#5a4bff", features: ["All tools incl. AI", "50 MB files", "150 ops/day", "Background removal", "Upscaling", "Unlimited batch"] },
  { plan: "team", label: "Team", monthly: 599, yearly: 479, color: "#457b9d", features: ["Everything in Pro", "200 MB files", "Unlimited ops", "10 members", "API access"] },
];
...
export default function UserDashboard({ onBack }) {
  ...
  const planColor = PLAN_COLORS[plan] || "#8a8496";
  ...
  const dailyLimit = { free: 8, pro: 150, team: null, enterprise: null }[plan];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <nav style={{ background: "var(--nav-bg)", borderBottom: "1px solid var(--border)", padding: "0 2rem", display: "flex", alignItems: "center", height: 60, position: "sticky", top: 0, zIndex: 100 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 15, color: "var(--text-secondary)" }}>← Back</button>
        <div style={{ flex: 1, textAlign: "center", fontWeight: 700, fontFamily: "'Fraunces', serif", fontSize: 18, color: "var(--brand)" }}>My Dashboard</div>
        <div style={{ width: 80 }} />
      </nav>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border)", padding: "1.75rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "1.5rem", flexWrap: "wrap" }}>
          {user?.avatar
            ? <img src={user.avatar} style={{ width: 64, height: 64, borderRadius: "50%", objectFit: "cover" }} />
            : <div style={{ width: 64, height: 64, borderRadius: "50%", background: "var(--brand)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 700 }}>{user?.name?.[0]?.toUpperCase()}</div>
          }
          <div style={{ flex: 1 }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: "var(--text-primary)" }}>{user?.usage_count || 0}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>total operations</div>
          </div>
        </div>

        <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border)", padding: "1.5rem", marginBottom: "1.5rem" }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)", marginBottom: "1rem" }}>Today's usage</div>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 8 }}>Resets at midnight IST</div>
        </div>

        {plan !== "enterprise" && (
          <div style={{ background: "var(--surface)", borderRadius: 16, border: "1px solid var(--border)", padding: "1.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>Upgrade your plan</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
              {UPGRADES.filter((u) => {
                ...
                <div key={u.plan} style={{ border: `2px solid ${u.color}`, borderRadius: 14, padding: "1.25rem" }}>
                  <div style={{ fontWeight: 700, fontSize: 16, color: u.color, marginBottom: 4 }}>{u.label}</div>
                  ...
                  <ul style={{ listStyle: "none", marginBottom: "1rem", display: "flex", flexDirection: "column", gap: 5 }}>
                    {u.features.map((f) => (
                      <li key={f} style={{ fontSize: 13, color: "var(--text-secondary)", display: "flex", gap: 7 }}>
